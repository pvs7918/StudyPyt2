...
# реализация с помощью декораторов
...
def save_to_json(res_dict) -> Callable:
    def wrapper():
        with open('task1.json', 'w', encoding='utf-8') as f1:
            json.dump(res_dict, f1, ensure_ascii=False, indent=4)
    return wrapper

def read_csv_and_decide_equations(func) -> Callable:
    #чтение строк из csv_file и вычисление по ним корней квадратных уравнений
    def wrapper() -> dict:
        res_dict = {}
        with open('task1.csv', 'r', newline='', encoding='utf-8') as f1:
            ...
            for i, row in enumerate(csv_file):
                if i != 0:
                    res_dict[str(row)] = func(row[0], row[1], row[2])
        return res_dict
    return wrapper
...
def main():
    ...
    # 2. Вызов декораторов для вычисления корней квадратных уравнений
    # вариант 1: в две строки
    # calc = read_csv_and_decide_equations(solving_quad_equation)
    # decide_and_save_to_json = save_to_json(calc())

    # вариант 2 - в одну строку (объединенный)
    decide_and_save_to_json = save_to_json(read_csv_and_decide_equations(
                                           solving_quad_equation)())
    decide_and_save_to_json()
