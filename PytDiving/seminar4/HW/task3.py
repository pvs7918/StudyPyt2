...
def atm_put_cash_to_account(atm: dict, client: dict, oper: dict, history: list):
    ...
    #записываем в историю операций словарь с парамертами операции
    history_dict={}
    history_dict["atm"] = atm
    history_dict["client"] = client
    history_dict["oper"] = oper
    history.append(history_dict)

    return True


def atm_dispense_cash_to_client(atm: dict, client: dict, oper: dict, history: list):
...
for oper in operations:
    ...
    if oper["type"].lower() == "внесение":
        atm_put_cash_to_account(oper_atm, oper_client, oper, history)
    if oper["type"].lower() == "снятие":
        atm_dispense_cash_to_client(oper_atm, oper_client, oper, history)
